import { E as R, C as z, s as G, t as $, L as w, a as q, b as v, e as b, i as p, c as u, h as E, g as C } from "./enibook-uiH8CL3S.js";
const Y = 148, X = 149, V = 150, N = 1, F = 2, T = 3, I = 151, D = 152, h = 4, L = 5, K = 153, k = 154, Z = 155, P = 156, _ = 6, U = 7, A = 8, B = 9, c = 0, o = [
  9,
  10,
  11,
  12,
  ...
  8287,
  12288
], J = 58, M = 40, f = 95, H = 91, l = 45, OO = 46, m = 35, $O = 37, x = 123, eO = 125, r = 47, S = 42, t = 10, j = 61, aO = 43, QO = 38;
function d(O) {
...
function y(O) {
  let e;
  return O.next == r && ((e = O.peek(1)) == r || e == S);
}
const nO = new R((O, e) => {
  if (e.dialectEnabled(c)) {
    let a;
    if (O.next < 0 && e.canShift(Z))
      O.acceptToken(Z);
    else if (((a = O.peek(-1)) == t || a < 0) && e.canShift(k)) {
      let Q = 0;
      for (; O.next != t && o.includes(O.next); )
        O.advance(), Q++;
      O.next == t || y(O) ? O.acceptToken(k, -Q) : Q && O.acceptToken(P);
    } else if (O.next == t)
      O.acceptToken(K, 1);
    else if (o.includes(O.next)) {
      for (O.advance(); O.next != t && o.includes(O.next); )
        O.advance();
      O.acceptToken(P);
    }
  } else {
    let a = 0;
    for (; o.includes(O.next); )
      O.advance(), a++;
    a && O.acceptToken(P);
  }
}, { contextual: !0 }), iO = new R((O, e) => {
  if (y(O)) {
    if (O.advance(), e.dialectEnabled(c)) {
      ...
      if (a > -1) {
        ...
        for (O.advance(); O.next >= 0; )
          ...
          } else if (Q && O.next == S && O.peek(1) == r) {
            n = 2;
            break;
          } else
            O.advance();
        O.acceptToken(Q ? U : _, n);
        return;
      }
    }
    if (O.next == r) {
      for (; O.next != t && O.next >= 0; )
        O.advance();
      O.acceptToken(_);
    } else {
      for (O.advance(); O.next >= 0; ) {
        let { next: a } = O;
        if (O.advance(), a == S && O.next == r) {
          O.advance();
          break;
        }
      }
  ...
  (O.next == aO || O.next == j) && e.dialectEnabled(c) && O.acceptToken(O.next == j ? A : B, 1);
}), oO = new R((O, e) => {
  if (!e.dialectEnabled(c))
    return;
  ...
  if (O.peek(-1) == t) {
    ...
    for (; O.next != t && o.includes(O.next); )
      O.advance(), n++;
    n != a && O.next != t && !y(O) && (n < a ? O.acceptToken(X, -n) : O.acceptToken(Y));
  }
}), RO = new R((O, e) => {
  for (let a = !1, Q = 0, n = 0; ; n++) {
    let { next: i } = O;
    if (d(i) || i == l || i == f || a && g(i))
      !a && (i != l || n > 0) && (a = !0), Q === n && i == l && Q++, O.advance();
    else if (i == m && O.peek(1) == x) {
      O.acceptToken(L, 2);
      break;
    } else {
      a && O.acceptToken(i == M ? I : Q == 2 && e.canShift(h) ? h : D);
      break;
    }
  }
}), lO = new R((O) => {
  if (O.next == eO) {
    for (O.advance(); d(O.next) || O.next == l || O.next == f || g(O.next); )
      O.advance();
    O.next == m && O.peek(1) == x ? O.acceptToken(F, 2) : O.acceptToken(N);
  }
}), rO = new R((O) => {
  if (o.includes(O.peek(-1))) {
    let { next: e } = O;
    (d(e) || e == f || e == m || e == OO || e == H || e == J || e == l || e == QO) && O.acceptToken(V);
  }
}), dO = new R((O) => {
  if (!o.includes(O.peek(-1))) {
    let { next: e } = O;
    if (e == $O && (O.advance(), O.acceptToken(T)), d(e)) {
      do
        O.advance();
      while (d(O.next));
      O.acceptToken(T);
...
const SO = new W(null, 0), cO = new z({
  start: SO,
  shift(O, e, a, Q) {
    return e == Y ? new W(O, a.pos - Q.pos) : e == X ? O.parent : O;
  },
  hash(O) {
    return O.hash;
  }
  ...
  "[ ]": $.squareBracket,
  "{ }": $.brace
}), XO = { __proto__: null, not: 62, only: 62, using: 173, with: 183, without: 183, hide: 197, show: 197, from: 220, to: 222, if: 235, through: 241, in: 247 }, sO = { __proto__: null, url: 80, "url-prefix": 80, domain: 80, regexp: 80, lang: 94, "nth-child": 94, "nth-last-child": 94, "nth-of-type": 94, "nth-last-of-type": 94, dir: 94, "host-context": 94, selector: 166 }, fO = { __proto__: null, "@import": 150, "@include": 170, "@mixin": 176, "@function": 176, "@use": 180, "@extend": 186, "@at-root": 190, "@forward": 194, "@media": 200, "@charset": 204, "@namespace": 208, "@keyframes": 214, "@supports": 226, "@if": 230, "@else": 232, "@for": 238, "@each": 244, "@while": 250, "@debug": 254, "@warn": 254, "@error": 254, "@return": 254 }, mO = w.deserialize({
  version: 14,
  ...
  tokenizers: [oO, rO, lO, dO, RO, nO, iO, tO, 0, 1, 2, 3],
  topRules: { StyleSheet: [0, 10], Styles: [1, 129] },
  dialects: { indented: 0 },
  specialized: [{ term: 152, get: (O) => XO[O] || -1 }, { term: 151, get: (O) => sO[O] || -1 }, { term: 74, get: (O) => fO[O] || -1 }],
  tokenPrec: 2821
}), s = /* @__PURE__ */ q.define({
  name: "sass",
...
}), TO = /* @__PURE__ */ E((O) => O.name == "VariableName" || O.name == "SassVariableName");
function kO(O) {
  return new C(O != null && O.indented ? yO : s, s.data.of({ autocomplete: TO }));
}
export {
  kO as sass,
  TO as sassCompletionSource,
  s as sassLanguage
};
